 ...
 * @returns { { diagnostics: Diagnostic[], output?: string, needlessDisables?: ({ diagnostic: Diagnostic, range: DisableReportRange })[], invalidScopeDisables?: ({ diagnostic: Diagnostic, range: DisableReportRange })[] } }
 */
function processResults(resultContainer, textDocument, ruleDocUrlProvider) {
	const { results, needlessDisables, invalidScopeDisables } = resultContainer;
	...
	if (needlessDisableSourceReport) {
		...
		for (const range of needlessDisableSourceReport.ranges) {
			const diagnostic = stylelintDisableOptionsReportRangeToVscodeDiagnostic(
				range,
				textDocument,
				'unused rule',
			);
 ...
 * @param {TextDocument} textDocument
 */
function stylelintDisableOptionsReportRangeToVscodeDiagnostic(range, textDocument, messagePrefix) {
	let message = messagePrefix + `: ${range.unusedRule}, start line: ${range.start}`;
